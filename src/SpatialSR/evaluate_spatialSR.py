...
import time
import os
from Temporal4DFlowNet.src.spatial_SR.Network.SR4DFlowNet import SR4DFlowNet
from Network.PatchGenerator import PatchGenerator
from utils import prediction_utils
from Temporal4DFlowNet.src.spatial_SR.utils.ImageDataset import ImageDataset
...
if __name__ == "__main__":
    ...
    gt_dir = '4DFlowNet/data/CARDIAC'
    result_dir = '4DFlowNet/results'
    lr_dir = '4DFlowNet/data/CARDIAC'

    gt_filename = 'M1_2mm_step5_static.h5'
    lr_filename = 'M1_2mm_step5_static_LR.h5'
    result_filename = 'test_result_MODEL1_2_new.h5'
    evaluation_filename = 'plots/eval_rel_err_1_2_new.h5'
    
    model_path = "4DFlowNet/models/4DFlowNet_20230203-1037/4DFlowNet-best.h5"
        ...
        with h5py.File(gt_filepath, mode = 'r' ) as h_gt:
            with h5py.File(lr_filepath, mode = 'r' ) as h_lr:
                ...
                for vel in vel_colnames:
                    gt[vel] = crop_gt(np.asarray(h_gt[vel]), h_pred[vel].shape)
                    pred[vel] = np.asarray(h_pred[vel])
                    lr[vel]= np.asarray(h_lr[vel])
                
                gt["mask"] = crop_gt(np.asarray(h_gt["mask"]), h_pred["u"].shape[1::])
    ...
    draw_reg_line(gt, pred, frame_idx= 10, save_as="4DFlowNet/results/plots/reg_example")
    plt.clf()
